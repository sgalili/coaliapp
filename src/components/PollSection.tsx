...
export const PollSection = ({
  ...
  const sortedOptions = Object.entries(results).sort(([, a], [, b]) => b.count - a.count).slice(0, 2);
  ...
  return <>
      <div className="-mx-4 border-t border-slate-200/50 bg-card shadow-sm !rounded-none">
        <div onClick={handleTogglePoll} className="w-full p-4 cursor-pointer hover:bg-muted/30 transition-all duration-200 rounded-none">
          {/* Header with title and action */}
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2 ml-2 px-[7px]">
              <Button variant="outline" size="sm" className="h-7 px-2 text-xs border-primary/20 text-primary hover:bg-primary/10" onClick={e => {
              e.stopPropagation();
              handleTogglePoll();
            }}>
          ...
          {/* Mini poll display */}
          {totalVotes > 0 && <div className="space-y-2">
              {/* Results preview */}
              <div className="flex items-center gap-4 text-xs text-muted-foreground mr-2">
                {topOption && <span>{topPercentage}% {topOption[0]}</span>}
                {secondOption && <span>•</span>}
                {secondOption && <span>{secondPercentage}% {secondOption[0]}</span>}
              </div>
              
              {/* Mini horizontal gauge */}
              <div className="relative h-2 bg-muted overflow-hidden">
                <div className="absolute right-0 top-0 h-full bg-primary transition-all duration-500" style={{
              width: `${topPercentage}%`
            }} />
                {secondOption && <div className="absolute top-0 h-full bg-secondary transition-all duration-500" style={{
              right: `${topPercentage}%`,
          ...
          {/* No votes yet state */}
          {totalVotes === 0 && <div className="text-xs text-muted-foreground text-center py-2">
      ...
      {showKYC && <KYCForm onSubmit={handleKYCSuccess} onBack={handleKYCClose} />}
    </>;
};