...
export const VideoFeedPage = ({
  ...
  const getVideos = (): VideoPost[] => {
    const baseVideos = activeFilter === 'candidates' ? [...mockCandidateVideos, ...mockExpertVideos] : mockCandidateVideos;
  ...
  return <div className="relative min-h-screen">
      ...
      <div className="fixed top-4 left-0 right-0 z-40 px-4">
        <VoteFilters activeFilter={activeFilter} onFilterChange={onFilterChange} />
      </div>

      {/* Create Content Button */}
      {onCreateContent && <button onClick={onCreateContent} className="fixed top-4 left-2 z-50 w-8 h-8 bg-muted/80 text-foreground rounded-full flex items-center justify-center shadow-sm hover:bg-muted transition-colors backdrop-blur-sm border border-border/20 mx-[11px]">
          <Plus className="w-4 h-4" />
        </button>}

      {/* Video Feed */}
      <VideoFeed posts={getVideos()} onTrust={handleTrustWithState} onWatch={handleWatchWithState} onZooz={handleZoozWithState} onVote={handleVoteWithState} userBalance={userBalance} currentUserId={currentUserId} isMuted={isMuted} onVolumeToggle={onVolumeToggle} />
    </div>;
};