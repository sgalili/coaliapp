        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "אתה עוזר חדשות שמחזיר את החדשות החשובות ביותר בפורמט JSON. החזר רק JSON תקין ללא טקסט נוסף."
                    },
                    {
                        "role": "user",
                        "content": f"""מצא את {max_results} החדשות הכי פופולריות והחשובות ביותר בנושא: {search_query}

החזר את התוצאות בפורמט JSON הבא בלבד:
{{
    "articles": [
        {{
            "title": "כותרת החדשה בעברית",
            "content": "תקציר של החדשה (2-3 משפטים)",
            "source": "שם המקור"
        }}
    ]
}}"""
